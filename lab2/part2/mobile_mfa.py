...
class BioConnect:
	...
	def getAuthenticatorStatus(self):
		...
		try:
			...
			#print(headers)
			#print(result.content)
			#sys.exit("Error: unexpected reply for get authenticator")
			#print("Error in get authenticator status")
			#print(reply)
			return('error')

		if reply.get("status","") == "active":
			if reply.get("face_status","") == "enrolled" or reply.get("voice_status","") == "enrolled" or reply.get("fingerprint_status","") == "enrolled" or reply.get("eye_status","") == "enrolled":
				return("active")
		return('inactive')
	...
	def sendStepup(self,
		...
		if result == False:
			...
			print(json.dumps(data))
			print(result.content)
			sys.exit("Error: in send setup")

		try:
			# Parse the json reply
			reply = json.loads(result.content.decode('utf-8'))
			
			# Extract the authentication tokens (Add self. here?)
			self.verificationId = reply.get("user_verification","").get("uuid","")
			#print("Verification ID: ", self.verificationId)
			#print("status: ", reply.get("user_verification","").get("status",""))

		except ValueError:
			self.verificationId = ""
			sys.exit("Bad verification ID")
	...
	def getStepupStatus(self):
		...
		try:
			...
			#print(headers)
			#print(result.content)
			#sys.exit("Error: Unexpected reply in Stepup Status")
